...
public abstract class EnhancedAutoMode extends LinearOpMode {
    ...
    public void makeActionObjects() throws Exception{
        ...
        if (!(xCoords.size() == yCoords.size() && xCoords.size() == angles.size() && xCoords.size() == methodIDs.size())) {
            dashboardTelemetry.addData("xCoords.size()", xCoords.size());
            dashboardTelemetry.addData("yCoords.size()", yCoords.size());
            dashboardTelemetry.addData("angles.size()", angles.size());
            dashboardTelemetry.addData("methodIDs.size()", methodIDs.size());
            throw invalidValues;
        }
