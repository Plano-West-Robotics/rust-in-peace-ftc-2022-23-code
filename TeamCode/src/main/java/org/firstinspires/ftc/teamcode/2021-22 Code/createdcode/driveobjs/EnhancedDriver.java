...
public class EnhancedDriver extends SampleMecanumDrive{
    ...
    private void executeAdvancedMovement(int id) throws IndexOutOfBoundsException{
        switch(id){
            ...
            case 1:
                Trajectory traj1 = trajectoryBuilder(getPoseEstimate())
                    .lineToLinearHeading(new Pose2d(getPoseEstimate().getX(), BLUE_WAREHOUSE_Y, 0))
                    .lineToLinearHeading(new Pose2d(BLUE_WAREHOUSE_X, BLUE_WAREHOUSE_Y, 0))
                    .build();
                followTrajectory(traj1);
                break;
            //moves to the rightmost edge (red side) and drives into warehouse
            case 2:
                Trajectory traj2 = trajectoryBuilder(getPoseEstimate())
                        .lineToLinearHeading(new Pose2d(getPoseEstimate().getX(), RED_WAREHOUSE_Y, 0))
                        .lineToLinearHeading(new Pose2d(RED_WAREHOUSE_X, RED_WAREHOUSE_Y, 0))
                        .build();
    ...
    private void moveArms(int encoderVal){
        armOne.setTargetPosition(-1*encoderVal);
        armTwo.setTargetPosition(-1*encoderVal);
